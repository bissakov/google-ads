...
def generate_query(fields: List[str], table: str, order_by: List[str]) -> str:
    metrics = [
        "metrics.average_cpv",
        "metrics.average_cpm",
        "metrics.cost_micros",
    ...
    conditions = [
        "segments.date BETWEEN '2023-08-01' AND '2024-04-28'",
    ]
    where_stmts = " AND ".join(conditions)
...
def fetch_general_metrics(
    client: GoogleAdsClient, accounts: List[GAccount]
...
def fetch_age_metrics(
    ...
    age_metrics = fetch_metrics(client, age_query, accounts, handle_age_metrics)
...
def fetch_geo_metrics(
    ...
    geo_metrics = fetch_metrics(client, geo_query, accounts, handle_geo_metrics)
